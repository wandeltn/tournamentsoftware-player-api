...
function getSearchResults(query: string): Promise<string | never[]> {
    ...
    return client.get(`/DoSearch?Page=1&SportID=2&Query=${encodeURIComponent(query)}&X-Requested-With=XMLHttpRequest&_=0`)
...
async function getPlayerDetails(name: string): Promise<PlayerDetails> {
    ...
    try {
        ...
        const playerName = $(
            "h5.media__title a.media__link span.nav-link__value"
        )
        .first()
        .text()
        .trim();
